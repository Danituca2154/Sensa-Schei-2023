	...
	def get_letter(self, img, lato):
		...
		if len(right_contour) != 0:
			...
			if diff < 7 and 75 < angle < 100:  # diff < 6.5
				prob_letter = 'H'
			elif 12 < diff < 33 and 75 < angle < 100:  # 10.5 < diff < 28
				prob_letter = 'U'
			elif 20 < diff < 60 and 10 < angle < 40:  # 20 < diff < 40
				prob_letter = 'S'
			...
			if prob_letter == 'H':
				if len(punti_ver) == 2 and (len(punti_or) == 2 or len(punti_or) == 1):  # len(punti_or) might be 1
					for point in punti_ver:
						if terzo_ver + mid_pt1[1] < point[1] < terzo_ver * 2 + mid_pt1[1]:  # se zona media
							counter += 1
					for point in punti_or:
						if len(punti_or) == 1:
							if terzo_or * 2 + mid_pt4[0] <= point[0] <= mid_pt2[0]:
								counter +=1
						elif len(punti_or) == 2:
